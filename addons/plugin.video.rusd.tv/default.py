...
PLUGIN_NAME = 'RuSD.TV'
SITE_HOSTNAME = 'www.rusd.tv'
SITEPREF      = 'http://%s' % SITE_HOSTNAME
SITE_URL      = SITEPREF + '/'

phpsessid_file = os.path.join(xbmc.translatePath('special://temp/'), 'plugin_video_rusd_tv.sess')
plotdescr_file = os.path.join(xbmc.translatePath('special://temp/'), 'plugin_video_rusd_tv.plot')
...
def ShowSeries(url):
	...
	for dat2 in http2:
		...
		row_img = row_img.replace('s_','b_')
		row_img = row_img.replace('futurama.jpg','futurama.png')
		row_img = row_img.replace('merlin.png','mer.png')
		row_img = row_img.replace('b_vm.png','b_vn.png')
...
def ShowEpisodes(url, title):
	...
	if http == None:
		...
		return
	Thumbnail = thumb
	picdat = re.compile('<div class="di"><img src="(.*?)"').findall(http)
	if len(picdat) > 0:
	...
	if len(seasons) > 0:
		for seanum, seacont in seasons:
			srows = re.compile('<div><span class="episode">(.*?)<a href="(.*?)">(.*?)</a></span></div>').findall(seacont)
			if len(srows) > 0:
				x = 1

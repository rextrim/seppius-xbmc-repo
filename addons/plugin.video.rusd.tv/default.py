...
def parse_playable_item(data):
	...
	return id_episodes,series,snum,enum,vnum,clean(title),tmark,enable,sub,smark,mark,server,vurl
...
def parse_series_item(data):
	...
	pimg  = SITE_URL + pimg
	return id_series,clean(title),clean(etitle),clean(info),fpimg,pimg,mark,enable,pos,isclosed

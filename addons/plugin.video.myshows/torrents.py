...
import urllib, socket, time, os, base64
    ...
    def download(self):
        ...
        if self.stype in ['json','btchat', 'torrent', 'multitorrent', 'rutracker', 'tpb', 'nnm', 'kz', 'torrenter', 'lostfilm']:
            ...
            if self.stype=='json':
                ...
                self.stype,self.ind,self.filename,self.title=j['stype'],j['ind'],j['filename'],urllib.unquote_plus(j['title'])
                nomagnet=True
            ...
            if success:
                ...
                if id:
                    if self.id:
                        ...
                        if self.title:
                            try:
                                filename_if=self.title[:len(self.title)-len(self.title.split('.')[-1])-1]
                                filename_if=os.path.split(filename_if)[-1]
                                for listfiles in Download().listfiles(id[0]):
                                    filename_if2=listfiles[0][:len(filename_if)]
                                    if listfiles[0].lower().split('.')[-1] in allowed_ext and\
                ...
                if ret>-1:
                    if not isRemoteTorr():
                        ...
                        if torrent_replacement!='':
                            ...
                            Debug('[Remote Torrent] Try to replace %s to %s in %s' % (dirname, torrent_replacement,self.filename))
    ...
    def DirAdd(self):
        ...
        if len(filename)>1:
            ...
            if self.addstype=='dir':
                ...
                except: showMessage(__language__(30206), __language__(30231) % (filename.decode('utf-8','ignore')), forced=True)
...
def LFSearch(showId, id):
    ...
    if lostlink:
        sdata=sdata+'&lostlink='+urllib.quote_plus(lostlink.replace('http://lostfilm.tv','http://www.lostfilm.tv'))
...
class MoveToXBMC(Source):
    def handle(self):
        ...
        if getSettingAsBool('alwayschoosemovemode'):
            ret = dialog.select(__language__(30297), [__language__(30027),__language__(30028),__language__(30029),__language__(30030), "SUBS COPY", __language__(30205)])
            if ret>=0 and ret<4: self.movemode=ret+1
            elif ret==4: self.movemode=10 #sub_copy
            else: self.movemode=None
        else:
        ...
        if self.movemode in (10,14) and self.stype in ['dir',' multifile']:
            self.subs_copy()
            return
    ...
    def move(self,folder,renamebool=True):
        ...
        if self.stype=='file':
            for i in range(0, len(filelist)):
                ...
                except: Debug('[move]: except for i='+str(i))
...
def xbmcEpisode(showId, seasonId, episodeNumber):
    ...
    for show in shows:
        if show['title']==title.decode('utf-8'):
            episodes = xbmcJsonRequest({'jsonrpc': '2.0', 'method': 'VideoLibrary.GetEpisodes', 'params': {'tvshowid': show['tvshowid'], 'season': seasonId, 'properties': ['episode', 'file']}, 'id': 0})

...
class RuTrackerOrg(SearcherABC.SearcherABC):
    ...
    def search(self, keyword):
        ...
        if None != response and 0 < len(response):
            ...
            for (forum, link, title, seeds) in re.compile('<a class="gen f" href="tracker\.php\?f=(\d+)">.+? class="med tLink hl-tags bold" href="\./viewtopic\.php\?t=(\d+)">(.+?)</a>.+?<td class="row4 seedmed"><b>(\d+)', re.DOTALL).findall(response):

...
def GET_old_OFF(target, referer, post_params = None, accept_redirect = True, get_redirect_url = False):
	try:
...
def upd_OFF(category, sort, text, n):
	try: n=str(int(n))
	except: n="0"
	if text=='0':
		stext=""
...
def Allre(http):
	...
	http=http.replace('<td class="details">\xc2\xe8\xe4\xe5\xee: <b>',chr(10)+"Flag1:['")
	http=http.replace('</b>. \xd0\xe0\xe7\xec\xe5\xf0: <b>',"','")
	http=http.replace('</b>.<br />\xcf\xe5\xf0\xe5\xe2\xee\xe4: <b>',"','")
	http=http.replace('</b>. \xd1\xea\xe0\xf7\xe0\xed: <b>',"','")
	http=http.replace('</b>. </td>\t<td align="left"><nobr><a href="',"','")
	http=http.replace('"><img src="http://lostfilm.tv/Tmpl/LostFilm/img',"']"+chr(10))
	L=http.splitlines()
	#debug(http)
	#debug(str(L))
	LL=[]
...
def GET_R(cse):
	...
	c,s,e=eval(cse)
	categoryUrl = xt('http://www.lostfilm.tv/rdr.php?c='+c+'&s='+s+'&e='+e)
...
def AllListS(L):
		for i in L:
			...
			if len(nm)==4: nm=nm+"  "
			try:
				Title = nm+" [COLOR FFFFFFFF]"+i[2]+"[/COLOR] / "+i[3]
				Title2 = nm+" "+i[2]+" / "+i[3]
				fnd=nm.find("[COLOR")
...
def clearinfo(str):
	...
	str=str.replace('О сериале',chr(10)+"Описание:")
...
def get_minfo(ntor):
			...
			try: dict=eval(xt(get_inf_db(ntor)[0][0]))#dbi = move_info_db[ntor]
				...
				for itm in LI:
					...
					if flag=='Обложка': 
						cvr=itm.strip()
						dict['cover']='http://www.lostfilm.tv'+itm[nc+1:].strip()
					...
					#	dict['plot']=ru(itm[nc+1:].strip()[:10])
...
def Search_OFF(category, sort, text, url='0'):
...
def OpenCat2_OFF(url, name, dict):
	ntor=dict["ntor"]
	L=get_qual(ntor)
	try:cover=dict["cover"]
	except:cover=""
...
elif mode == 'Search_OFF':
	Search(category, sort, text, url)
	xbmcplugin.setPluginCategory(handle, PLUGIN_NAME)
	xbmcplugin.endOfDirectory(handle)

elif mode == 'play_url2_OFF':
	play_url2(params)

elif mode == 'OpenCat2_OFF':

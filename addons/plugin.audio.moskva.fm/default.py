...
def OpenStations(ci):
	...
	for i in L1:
			...
			try:
				Li=eval(i)
				url = Li[6]
				n=Li[0].find("http://www.moskva.fm")
				url = Li[0][n:]
				print url
				img = Li[2]
				if len(Li[5])<5: dbr="  "
...
def PlayStation_old(id,name,img):
...
def GetLink(link):
	...
	#print http
	ss="var streams = ["
	es='src="http://css.moskva.fm/js/jplayer/jplayer/jquery.jplayer.js'
	slinks=mfindal(http, ss, es)[0]
	ss="http://"
	es="',"
	Llinks=mfindal(slinks, ss, es)
	streamlink=Llinks[0]
	return streamlink

...
class Scrobbler(threading.Thread):
	...
	def watching(self):
		...
		if self.curVideo['type'] == 'episode':
				...
				if self.curVideo.has_key("multi_episode_count"):
					...
					if cur_episode > 0:
						if not self.markedAsWatched[cur_episode - 1]:
							...
							response = self.myshowsapi.scrobbleEpisode(episode_data['tvdb_id'], episode_data['showtitle'], episode_data['year'], episode_data['season'], episode_data['episode'], episode_data['uniqueid']['unknown'], self.totalTime/60, 100)
							if response != None:
			...
			#response = self.myshowsapi.watchingEpisode(match['tvdb_id'], match['showtitle'], match['year'], match['season'], match['episode'], match['uniqueid']['unknown'], duration, watchedPercent)
			#if response != None:
	...
	def scrobble(self):
		...
		if self.curVideo['type']:
			...
			xbmc.executebuiltin('xbmc.RunPlugin("plugin://plugin.video.myshows/?mode=70&action=check&title='+urllib.quote_plus(json.dumps(match))+'")')
	...
	def check(self):
		...
		Debug("[Scrobbler] watched: %s / %s, min=%s" % (str(self.watchedTime), str(self.totalTime), str(scrobbleMinViewTimeOption)))

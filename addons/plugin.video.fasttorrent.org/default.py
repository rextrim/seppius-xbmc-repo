...
def upd2(pr,n=1):
	...
	post = "search_page="+str(n)+"&search_pages=50&keyword="+keyword+"&sort=8&up_celebrity=0"#
...
def Search(category, sort, text, pr):
	...
	for n in range (NP,IT):
		...
		if n == NP and text=="0":
			try:
				if category.find("all")!=-1:        newcategory="new/all"
				if category.find("films")!=-1:      newcategory="new-films"
				if category.find("tv")!=-1:         newcategory="last-tv-torrent"
				if category.find("multfilm")!=-1:   newcategory="new-multfilm-movies"
				if category.find("documentary")!=-1:newcategory="new/documentary"
				if category.find("music")!=-1:      newcategory="new/music"

...
def checkScrobblingExclusion(fullpath):
    ...
    if (fullpath.find("http://") > -1) and get_bool_setting("ExcludeHTTP"):
        Debug(
            "checkScrobblingExclusion(): Video is playing via HTTP source, which is currently set as excluded location.")
        return True
    ...
    if ExcludePath != "" and get_bool_setting("ExcludePathOption"):
        if (fullpath.find(ExcludePath) > -1):
            Debug(
                'checkScrobblingExclusion(): Video is playing from location, which is currently set as excluded path 1.')
            return True
    ...
    if ExcludePath2 != "" and get_bool_setting("ExcludePathOption2"):
        if (fullpath.find(ExcludePath2) > -1):
            Debug(
                'checkScrobblingExclusion(): Video is playing from location, which is currently set as excluded path 2.')
            return True
    ...
    if ExcludePath3 != "" and get_bool_setting("ExcludePathOption3"):
        if (fullpath.find(ExcludePath3) > -1):
            Debug(
                'checkScrobblingExclusion(): Video is playing from location, which is currently set as excluded path 3.')
            return True
...
def getEpisodeDetailsFromXbmc(libraryId, fields):
    rpccmd = json.dumps({'jsonrpc': '2.0', 'method': 'VideoLibrary.GetEpisodeDetails',
    ...
    try:
        # get tvdb id
        rpccmd_show = json.dumps({'jsonrpc': '2.0', 'method': 'VideoLibrary.GetTVShowDetails',
                                  'params': {'tvshowid': result['result']['episodedetails']['tvshowid'],
                                             'properties': ['year', 'imdbnumber']}, 'id': 1})
...
def getMovieDetailsFromXbmc(libraryId, fields):
    rpccmd = json.dumps({'jsonrpc': '2.0', 'method': 'VideoLibrary.GetMovieDetails',
                         'params': {'movieid': libraryId, 'properties': fields}, 'id': 1})
...
def getPlaylistLengthFromXBMCPlayer(playerid):
    ...
    if playerid < 0 or playerid > 2:
        ...
        return 0
    rpccmd = json.dumps({'jsonrpc': '2.0', 'method': 'Player.GetProperties',
                         'params': {'playerid': playerid, 'properties': ['playlistid']}, 'id': 1})
    result = xbmc.executeJSONRPC(rpccmd)
    result = json.loads(result)
    # check for error
    try:
    ...
    rpccmd = json.dumps({'jsonrpc': '2.0', 'method': 'Playlist.GetProperties',
                         'params': {'playlistid': playlistid, 'properties': ['size']}, 'id': 1})
    result = xbmc.executeJSONRPC(rpccmd)
    result = json.loads(result)
    # check for error
    try:

...
class Link:
    def __init__(self, mode, argv=None, container=False, container_replace=False):
        app = {'mode': mode}
        if argv is not None:
            app['argv'] = argv
        if container:
...
class Handler:
    def __init__(self, link, argv=None):
        self.argv = argv
        self.link = link
    ...
    def run(self, **kwarg):
        ...
        if 'fanart' in kwarg:
            if kwarg['fanart'] and isinstance(kwarg['fanart'], basestring):
                ...
                fanart = xbmcaddon.Addon(id=sys.argv[0].replace('plugin://', '').replace('/', '')).getAddonInfo(
                    'fanart')
            if fanart:
        ...
        try:
            ...
            xbmc.log('XBMCup: Plugin: error exec handler: ' + str(e) + '(sys.argv: ' + str(sys.argv) + ')',
                     xbmc.LOGERROR)
            raise

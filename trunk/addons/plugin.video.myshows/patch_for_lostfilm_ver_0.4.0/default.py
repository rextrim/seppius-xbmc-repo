...
print '[SYS ARGV]: '+str(urllib.unquote_plus(sys.argv[2]))[1:]
...
def GET_R(cse):
	#print cse
	c,s,e=eval(cse)
	categoryUrl = xt('http://www.lostfilm.tv/nrdr.php?c='+c+'&s='+s+'&e='+e)
...
def AllListN(L):
		...
		for i in L:
			...
			sn = i[5]
			dict=get_minfo(row_url)
			try:cover=dict["cover"]#"http://www.lostfilm.tv/Static/icons/"+i[3]
			except:cover=""
			listitem = xbmcgui.ListItem(Title, thumbnailImage=cover, iconImage=cover)
			listitem.setInfo(type = "Video", infoLabels = {"Title": Title} )
			try:listitem.setInfo(type="Video", infoLabels=dict)
...
def Allrelis(L):
    if mode!='myshows':
        ...
        for i in L:
            ...
            myshows_files.append(row_url)
        dialog = xbmcgui.Dialog()
        if len(myshows_items)==1: ret=0

...
def rateMedia(media_id, media_type):
	...
	if media_type == 'movie':
		...
		myshows_summary = globals.myshowsapi.getMovieSummary(xbmc_media['imdbnumber'])
		if myshows_summary == None:
			Debug('[Rating] Failed to retrieve movie data from myshows')
			return

		if myshows_summary['rating'] or myshows_summary['rating_advanced']:
		...
		#myshows_summary = globals.myshowsapi.getShowSummary(xbmc_media['imdbnumber'], xbmc_media['episode']['season'], xbmc_media['episode']['episode'])
		#if myshows_summary == None:
		#	Debug('[Rating] Failed to retrieve show/episode data from myshows')
		#	return

		#xbmc_media['year'] = myshows_summary['show']['year']

		#if myshows_summary['episode']['rating'] or myshows_summary['episode']['rating_advanced']:
		Debug('[Rating] Episode has been rated')
		return

	if not globals.myshowsapi.settings:
		globals.myshowsapi.getAccountSettings()
	rating_type = globals.myshowsapi.settings['viewing']['ratings']['mode']
	...
	gui.doModal()
	if gui.rating:
		rateOnmyshows(gui.rating, gui.media_type, gui.media)
	del gui


def rateOnmyshows(rating, media_type, media):
	Debug('[rating] Sending rating (%s) to myshows' % rating)
	if media_type == 'movie':
		...
		data = globals.myshowsapi.rateMovie(params)
		...
		data = globals.myshowsapi.rateEpisode(params)

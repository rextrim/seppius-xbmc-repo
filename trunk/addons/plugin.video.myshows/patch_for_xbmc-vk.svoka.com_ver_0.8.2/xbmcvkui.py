...
class XBMCVkUI_VKSearch_Base(XBMCVkUI_Search_Base):
    ...
    def Search(self,query):
        try:
            ...
            stringdata = self.params.get("stringdata")
        except: pass
        ...
        if result:
            if external=='1':
                ...
                myshows_files=[]
                for a in result:
                    listing=self.ProcessFoundEntry(a)
                    myshows_items.append(listing[0])
                    myshows_files.append(listing[1])
                myshows_items.append(unicode(myshows_lang(30400)))
                myshows_files.append('plugin://plugin.video.myshows/?mode=203&stringdata='+stringdata)

                print 'files: '+str(myshows_files)
                dialog = xbmcgui.Dialog()
                ret = dialog.select(unicode(myshows_lang(30401)), myshows_items)
		if ret>-1:
               	    xbmc.executebuiltin('xbmc.RunPlugin("'+myshows_files[ret]+'&external=1&sdata='+self.params.get("sdata")+'")')
            else:
                for a in result:
                    self.ProcessFoundEntry(a)
        elif external=='1':
            dialog = xbmcgui.Dialog()
            dialog.ok(unicode(myshows_lang(30402)), unicode(myshows_lang(30403)), unicode(myshows_lang(30404)))
            xbmc.executebuiltin('xbmc.RunPlugin("plugin://plugin.video.myshows/?mode=203&stringdata='+stringdata+'")')

...
def showMessage(heading, message, times=10000, forced=False):
    ...
    if debug == 'true' or notification == 'true' or forced:
        ...
        heading.encode('utf-8').replace('"', "'"), unicode(message).replace('"', "'").encode('utf-8'), times, icon))


def id2title(showId, id=None, norus=False):
    jload = Data(cookie_auth, __baseurl__ + '/shows/' + str(showId)).get(force_cache=True)
    if jload:
...
def date2SE(showId, date):
    jload = Data(cookie_auth, __baseurl__ + '/shows/' + str(showId)).get()
    if jload:
        jdata = json.loads(jload)
...
def fdate_bigger_ldate(fdate, ldate):
    if int(fdate.split('.')[2]) > int(ldate.split('.')[2]):
        ...
        return True
    elif int(fdate.split('.')[2]) == int(ldate.split('.')[2]) and int(fdate.split('.')[1]) == int(
            ldate.split('.')[1]) and int(fdate.split('.')[0]) > int(ldate.split('.')[0]):
        return True
    else:
        return False
...
def auth():
    ...
    if len(passwd) != 32 and passwd != '':
        ...
        passwd = __settings__.getSetting("password")
    url = __baseurl__ + '/profile/login?login=' + login + '&password=' + passwd
    ...
    try:
        ...
        if e.code in (403, 404) or not login or login == '' or not passwd or passwd == '':
            if __settings__.getSetting("lastlogin") == '' or float(time.time()) - float(
                    __settings__.getSetting("lastlogin")) > 60 * 1000:
...
def lockView(viewId='list'):
    ...
    try:
        xbmc.executebuiltin("Container.SetViewMode(%s)" % str(skinOptimizations[0][viewId]))
    except:
        return
...
def uTorrentBrowser():
    ...
    try:
        ...
        action = None
    try:
        hash = urllib.unquote_plus(apps['argv']['hash'])
    except:
        hash = None
    try:
        ind = urllib.unquote_plus(apps['argv']['ind'])
    except:
        ind = None
    try:
        tdir = urllib.unquote_plus(apps['argv']['tdir'])
    except:
        tdir = None
    ...
    if action:
        ...
        if action == 'search' and hash:
            ...
            if title:
                ...
                if ret > -1:
                    xbmc.executebuiltin(
                        'ActivateWindow(Videos,plugin://plugin.video.myshows/?mode=19&action=%s)' % (titles[ret]))
            return
        if (ind or ind == 0) and action in ('0', '3'):
            ...
            for data in dllist:
                if data[2] == int(ind):
                    ...
                    break
            if isRemoteTorr():
                folder = folder.replace(__settings__.getSetting("torrent_dir"),
                                        __settings__.getSetting("torrent_replacement"))
    ...
    if not hash:
        for data in DownloadList:
            ...
            menu.append(
                {"title": '[' + str(data['progress']) + '%]' + status + data['name'] + ' [' + str(data['ratio']) + ']',
                 "mode": "52", "argv": {'hash': str(data['id'])}})
    elif not tdir:
        dllist = sorted(Download().listfiles(hash), key=lambda x: x[0])
        for name, percent, ind, size in dllist:
            if '/' not in name:
                menu.append({"title": '[' + str(percent) + '%]' + '[' + str(size) + '] ' + name, "mode": "52",
        ...
        for name, percent, ind, size in dllist:
            if '/' in name and tdir in name:
                menu.append(
                    {"title": '[' + str(percent) + '%]' + '[' + str(size) + '] ' + name[len(tdir) + 1:], "mode": "52",
                     "argv": {'hash': hash, 'ind': str(ind), 'action': 'context'}})
    ...
    for i in menu:
        ...
        popup = []
        if not hash:
            actions = [('start', __language__(30281)), ('stop', __language__(30282)), ('remove', __language__(30283)),
                       ('search', __language__(30101)), ('3', __language__(30320)), ('0', __language__(30321)),
                       ('removedata', __language__(30284))]
            folder = True
        else:
...
class PluginStatus():
    def __init__(self):
        self.patchfiles = [('myshows', 'script.myshows', 'script.myshows',
                            ['notification_service.py', 'utilities.py', 'service.py', 'scrobbler.py']),
                           ('vkstatus', 'xbmc-vk.svoka.com', 'patch_for_xbmc-vk.svoka.com_ver_1.1.0',
                            ['xbmcvkui.py', 'xvvideo.py']),
                           ('lostfilm', 'plugin.video.LostFilm', 'patch_for_lostfilm_ver_0.4.3',
                            ['default.py', 'Downloader.py']),
                           ('torrenterstatus', 'plugin.video.torrenter', 'patch_for_plugin.video.torrenter_ver_1.2.7',
                            ['Core.py', 'Downloader.py', 'resources/searchers/RuTrackerOrg.py',
                             'resources/searchers/ThePirateBaySe.py',
    ...
    def menu(self):
        ...
        menu = [{"title": __language__(30142) % len(TorrentDB().get_all()), "mode": "50", "argv": {'action': ''}},
                ...
                {"title": unicode(__language__(30545)) % TimeOut().timeout(), "mode": "61",
                 "argv": {'action': 'timeout'}},
                ...
                {"title": 'script.module.torrent.ts (ACE TStream): %s' % TSstatus, "mode": "61",
                 "argv": {'action': 'tscheck'}},
                {"title": 'Python-LibTorrent: %s' % libmode, "mode": "61", "argv": {'action': 'libmode'}},
                {"title": 'plugin.video.torrenter: %s' % self.torrenterstatus, "mode": "61",
                 "argv": {'action': 'torrenterstatus'}},
        ...
        for i in menu:
            ...
            if i['argv']['action'] in self.status and self.status[i['argv']['action']] == 'NOT PATCHED':
                h.item(link, title=unicode(i['title']),
                       popup=[(Link(i['mode'] + '0', i['argv']), unicode(__language__(30286)))], popup_replace=True)
            elif i['argv']['action'] in self.status and self.status[i['argv']['action']] == 'NOT INSTALLED':
                h.item(link, title=unicode(i['title']),
                       popup=[(Link(i['mode'] + '1', i['argv']), unicode(__language__(30316)))], popup_replace=True)
            else:
                h.item(link, title=unicode(i['title']))
    ...
    def check_status(self, id, patchpath, filelist):
        ...
        try:
            os.path.getsize(os.path.join(plugpath, filelist[0]))
        except:
            try:
                ...
                return "NOT INSTALLED"
        try:
            os.path.getsize(os.path.join(__addonpath__, patchpath, filelist[0]))
        except:
            __addonpath__ = xbmcaddon.Addon(id='plugin.video.myshows').getAddonInfo('path').decode('utf-8').encode(
                'cp1251')
        for f in filelist:
            try:
    ...
    def install(self, action):
        ...
        for f in filelist:
            try:
                success = shutil.copyfile(os.path.join(__addonpath__, patchpath, f), os.path.join(plugpath, f))
            except:
                plugstatus = True
        xbmc.executebuiltin('Container.Refresh')
        if not plugstatus:
            showMessage(__language__(30286), __language__(30208))
        else:
            showMessage(__language__(30286), __language__(30206), forced=True)
...
def season_banner(banners, season):
    ...
    try:
        season_banners = [banner for banner in banners if
                          banner["bannertype"] == "season" and int(banner["season"]) == season]
    except:
        season_banners = []
...
def auth_xbmc(login=None, passwd=None):
    ...
    try:
        ...
        Debug('[auth]: url - ' + str(url) + '; HTTPError - ' + str(e) + ' ;e -' + str(e.code), True)
        if e.code in (403, 404) or not login or login == '' or not passwd or passwd == '':
            if __settings__.getSetting("lastlogin") == '' or int(time.time()) - int(
                    __settings__.getSetting("lastlogin")) > 60 * 1000:
                __settings__.setSetting("lastlogin", str(round(time.time())))
                dialog = xbmcgui.Dialog()
                ok = dialog.yesno(__language__(30201), __language__(30201), __language__(30202))

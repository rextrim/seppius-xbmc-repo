...
def KP(nType, nGenre, Rating, nYar, nCantry, text):
	...
	if text!="1": Url = xt('http://www.kinopoisk.ru/level/7/type/film/list/1/find/'+text+'/')#%EF%E8%F0%EE%E3/')
...
def Popular():
	httpUrl="http://www.kinopoisk.ru"
	Url = xt('http://www.kinopoisk.ru/level/56/')
	http = GET(Url, httpUrl, None, True, False, 'www.kinopoisk.ru')
	if http == None:
		showMessage('kinopoisk:', 'Сервер не отвечает', 1000)
		return None
		...
		for tTitle in RootList:
			if len(tTitle)>0:
				rtk=tTitle[0]
				title=tTitle[2]
				cover="http://st.kinopoisk.ru/images"+tTitle[1][9:-2]+".jpg"
				n=tTitle[2].rfind("(")
				k=tTitle[2].rfind(")")
				...
				Title = "| "+rtk+" |  "+tTitle[2].strip()#+"I"+ortitle+"I"
				if 1==1:

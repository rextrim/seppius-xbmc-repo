...
def Allre(http):
	...
	http=http.replace('</b>. </td>	<td align="left"><nobr><img src="http://lostfilm.tv/Tmpl/LostFilm/img/icon_free.gif" align="center" />&nbsp;<a href="',"','")
	http=http.replace('</b>. </td>\t<td align="left"><nobr><img src="http://lostfilm.tv/Tmpl/LostFilm/img/icon_tv_release.gif" align="center" />&nbsp;<a href="',"','")
...
def AllListN(L):
		if 1==1:
			Title = "[COLOR FF00FF00][Полный список сериалов][/COLOR]"
			listitem = xbmcgui.ListItem(Title)
			listitem.setInfo(type = "Video", infoLabels = {"Title": Title} )
		...
		for i in L:
			#nm=i[2].lower()
			Title = format_s(i[0]+" [B][COLOR FFFFFFFF]"+i[1]+":[/COLOR][/B] "+i[6])
			row_url = i[2]
			sn = i[10]
			dict=get_minfo(row_url)
			try:cover=dict["cover"]

...
def mainMain(params):
    ...
    xbmcplugin.addDirectoryItem(hos, uri, listitem, True)
    beautifulSoup = BeautifulSoup(http)
    content = beautifulSoup.find('ul',attrs={'class':'menu2'})
...
def get_movies(params):
    ...
    print beautifulSoup
    m=re.findall('</font>(.+?)</font></i><br /><br /><iframe src="(.+?)"',str(beautifulSoup))
    if not m:
        m=re.findall(' </font>(.+?)</font><br /><br /><iframe src="(.+?)"',str(beautifulSoup))
        if not m:
            m= re.findall(' <font>(.+?)</font> онлайн в хорошем качестве</font><br /><br /><iframe src="(.+?)"',str(beautifulSoup))
    for n in m: 

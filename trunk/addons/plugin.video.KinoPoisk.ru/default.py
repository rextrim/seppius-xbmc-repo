...
def AddItem(Title = "", mode = "", info = {"cover":icon}, total=100):
			...
			if mode=="Torrents": 
				...
				listitem.addContextMenuItems([('Hайти похожие', 'XBMC.ReplaceWindow(10025,"plugin://plugin.video.KinoPoisk.ru/?mode=Recomend&id='+id+'")'), ('Персоны', 'XBMC.ReplaceWindow(10025,"plugin://plugin.video.KinoPoisk.ru/?mode=Person&id='+id+'")')])
...
#		if len(i)>10:
			...
			if nbl <> "":
				...
				n1=nen.rfind(",")
				duration = nen[n1+1:-4].replace(" ", "")
				
				tmp=nen[:n1]
				n2=tmp.rfind(",")
				if n2 <0 and nen[-1:]<>".": 
					try:year = int(nen[n1+1:])
					except: year = None
					duration = ""
				else:
					year = int(tmp[n2+1:].replace(" ", ""))
...
if mode == "Torrents":
	...
	else:text = params["info"]["originaltitle"][:-1]+" "+str(params["info"]["year"])+" "+rus.replace("a","а")
	n=text.find("(")
